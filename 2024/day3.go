...
func GetSumofDisableableMultipieldCorruptedMemory(scanner *bufio.Scanner) int {
	sum := 0
	isDisabled := false
	for scanner.Scan() {
		line := scanner.Text()
		pairs := getDisableablePairs(line, &isDisabled)
...
func getDisableablePairs(line string, isDisabled *bool) []MultiplicationPair {
	var pairs []MultiplicationPair
	regex, err := regexp.Compile(`do\(\)|don't\(\)|mul\([0-9]+,[0-9]+\)`)
	if err != nil {
		log.Fatalf("regexp.Compile: %s", err)
	}
	found := regex.FindAllString(line, -1)
	if len(found) > 0 {
		for i := 0; i < len(found); i++ {
			...
			if strings.Contains(found[i], "mul") {
				intermediate := strings.Split(found[i], "(")
				intermediate = strings.Split(intermediate[1], ")")
				pair := strings.Split(intermediate[0], ",")
				if len(pair) == 2 {
