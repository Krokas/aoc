package aoc2024

import (
	"bufio"
	"strings"
	"testing"
)

var printingManualSample string = `47|53
97|13
97|61
97|47
75|29
61|13
75|53
29|13
97|29
53|29
61|53
97|53
61|29
47|13
75|47
97|75
47|61
75|61
47|29
75|13
53|13

75,47,61,53,29
97,61,53,29,13
75,29,13
75,97,47,61,53
61,13,29
97,13,75,29,47`

func TestGetSumOfMiddlePageNumbers(t *testing.T) {
	scnaner := bufio.NewScanner(strings.NewReader(printingManualSample))
	got := GetSumOfMiddlePageNumbers(scnaner)
	if got != 143 {
		t.Errorf("GetSumOfMiddlePageNumbers = %d, expected 143", got)
	}
}

func TestGetSumOfIncorrectlyOrderedUpdateMiddlePages(t *testing.T) {
	scanner := bufio.NewScanner(strings.NewReader(printingManualSample))
	got := GetSumOfIncorrectlyOrderedUpdateMiddlePages(scanner)
	if got != 123 {
		t.Errorf("GetSumOfIncorrectlyOrderedUpdateMiddlePages = %d, expected 123", got)
	}
}